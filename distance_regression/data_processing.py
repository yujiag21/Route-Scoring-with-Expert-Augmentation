...
def data_process(distances_dict, mode='sdf'):
    '''
    Process the routes dictionary into a dataframe with embedding the reactions into feasibility_input and inputs
    '''

    start = datetime.now()
    # 构建行列表，避免逐条 append 带来的性能/弃用问题
    rows = []
    for key, value_list in distances_dict.items():
        for entry in value_list:
            rows.append({'key': key, 'values': entry})
    ...
    reaction_class = pd.read_csv(os.path.join(ROOT_DIR, 'reaction_class_summ_20.csv'))
