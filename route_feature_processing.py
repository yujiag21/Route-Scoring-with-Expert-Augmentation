...
class WeightedScorer(Scorer):
    ...
    def _calculate_tree_costs(
        ...
        # print([mol.smiles for mol in list(costs.keys())], [mol.smiles for mol in list(complexity.keys())])
        return defaultdict(lambda: max_cost * self.not_in_stock_multiplier, costs), complexity
    ...
    def _score_node(self, node):
        ...
        volume = sum([prices[mol] for mol in prices.keys()])
        complexity = sum([stabilities[mol] for mol in stabilities.keys()])

        return [volume,complexity,feasibility,reaction_list]

    def _score_reaction_tree(self, tree):
        print('use score_reaction_tree')
        reaction_class = pd.read_csv('reaction_class_summ_20.csv')
        feasibility = 0
        ...
        for reaction in tree.reactions():
            try:
                ...
                continue
        leaf_costs, complexity = self._calculate_tree_costs(tree)
        volume = sum([leaf_costs[leaf] for leaf in leaf_costs.keys()])
        try:
            # complexity = sum([complexity[leaf] for leaf in tree.leafs()])
            complexity = sum([complexity[leaf] for leaf in complexity.keys()])
        except KeyError:
            complexity = 0
        return [volume,complexity,feasibility,reaction_list]
...
def main(args):
    ...
    with open(f'{args.input_file}', 'r') as datafile:
        data = datafile.read()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_file", type=str, default='data/data_assessment/routes_assessment_step_scores.json', help="Input file name")
    parser.add_argument("--output_file", type=str, default='data/data_assessment/processed_routes_assessment_step_scores.json', help="Output save file")
